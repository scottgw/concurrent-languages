class THRESH_PARFOR_WORKER
create make
feature
  make (nrows_, ncols_: INTEGER;
      from_array_, to_array_: separate ARRAY[INTEGER];
      threshold_: INTEGER;
      aggregator_: separate THRESH_PARFOR_AGGREGATOR)
  local
  do
    nrows := nrows_
    ncols := ncols_
...
feature
  ...
  put_result(an_aggregator: separate THRESH_PARFOR_AGGREGATOR)
  do
    an_aggregator.put
  end
...
feature {NONE}
  ...
  threshold: INTEGER
  aggregator: separate THRESH_PARFOR_AGGREGATOR
